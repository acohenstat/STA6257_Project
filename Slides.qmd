...
The approach is the same as regression imputation except instead of random draws from a conditional distribution, missing values
are based on predicted values of the outcome variable.
...
either random or deterministic, which is based on a metric or value. It does not rely on model fitting.
...
imputed value. This maintains the variability of the data.
...
and a prediction set. The missing values are then replaced
...
-   According to Rubin's Rule, in multiple imputation missing values are replaced by *m* imputed values which result in *M* complete
datasets. 
-   For each of the *M* datasets, an estimate of $\theta$ is
acquired. 
...
-   Let ${\hat{\theta}}_{m}$ and ${\hat{\phi}}_{m}$ be an
estimator of the variance of ${\hat{\theta}}_{m}$ based on the *M*th
complete dataset.

-   To calculate the pooled mean differences of $\theta$:

$${\overline{\theta}} = \displaystyle \frac{1}{m}(\sum_{i = 1}^{m} {{\theta}}_{i})$$

  -   $\overline\theta$ = pooled parameter estimate 
  -   m = number of imputed datasets
  -   $\theta_m$ = sum of the parameter estimate in each imputed dataset i
      

## {.scrollable}
-   To calculate the proposed variance estimator of ${\hat{\theta}}_{m}$:

$${\hat{\Phi}}_{i} = {\overline{\phi}}_{i} + (1+\displaystyle \frac{1}{m})B_{m}$$
where the average within imputation variance (normal standard error) is:

$${\overline{\phi}}_{m} = \displaystyle \frac{1}{m}\sum_{}^{}{SE}^2$$

-   It will be large in small samples and small in large samples.
  -   m = number of imputed datasets
  -   $SE^2$ = sum of the standard error squared in each imputed dataset i
  
## 

with a correction factor of $(1+\displaystyle \frac{1}{M})$,

and the between imputation variance (how much variation because of the imputation process) is:

$$B_{m} = \displaystyle \frac{1}{m-1}\sum_{i = 1}^{m}({\hat{\theta}}_{i}-{\overline{\theta}}_{})^{2}$$
  -   $\overline\theta$ = pooled parameter estimate 
  -   m = number of imputed datasets
  -   $\theta_i$ = parameter estimate in each imputed dataset i
...
-   The parameters ${\theta}$ of the data model and the 
    parameters ${\phi}$ of the model for the missing values are distinct.
    That is, knowing the values of ${\theta}$ does not provide any information about ${\phi}$.
