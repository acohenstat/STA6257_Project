...
imputations, etc.) [@ped2017]. According to [@wul2017], 3-5 imputations are sufficient, and 10 are more than enough.
There are two types of imputation: single
and multiple.
...
=======
Using the MICE (Multivariate Imputation by Chained Equations) method found in the MICE package for R which creates multiple datasets with imputted values for the missing values, 5 datasets where plausible values are installed into the intersecctions of rows and columns with missing data were generated. 

An array of statistical analysis steps using R coding was used to imputate missing values of the credit data.
...
library(gtsummary)
```
In order to perform multiple imputation on categorical data, all stringvariables must be converted to \# factors using as.factor()
...
>>>>>>> 0b8583752f1607500520bf84da92fa1e9336034a
...
We now must analyze the data to see how we intend to handle the missing values. In order to do this, we need to create a new dataset, called new_credit, that deletes the missing data. We want to perserve the original dataset so we can implement the method we intend to use to address the missing values. We also want to remove any categorical variables that have missing values, as we cannot impute those using multiple imputation. We can then generate a count of rows to determine how many values were deleted.
...
incorrect statistical model will result.

```{r, warning=FALSE, echo=T, message=FALSE}
# Need to add linear regression check
```
...
Blue is observed values and red is missing values. There are 11 patterns. (Need to add better explanation.)
...
fit <- with(Multiple_Imputation, lm(Amount ~ Seniority + Home + Time + Age + Marital + Records + Job + Expenses + Income + Assets + Debt + Status + Price))
