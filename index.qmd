...
[@ped2017]. According to [@wul2017], 3-5 imputations are sufficient, and
10 are more than enough.
...
Need to add assumptions here.

#### Steps in Multiple Imputation Method

The chained equation process has the following steps [@azu2011]:

*Step 1:* Using simple imputation, replace the missing data with this value,
referred to as the "place holder".

*Step 2:* The "place holder" values for one variable are set back to missing.

*Step 3:* The observed values from this variable (dependent variable) are
...
*Step 4:* The missing values are replaced with predictions "m" from this newly
created model.

*Step 5:* Repeat Steps 2-4 for each variable that have missing values until all
missing values have been replaced.

*Step 6:* Repeat Steps 2-4, updating imputations each cycle for as many "m"
...
```{r, warning=FALSE, echo=T, message=FALSE}
# load data
gpa = read.csv("gpa_study_hours.csv")
gpa_missing = read.csv("gpa_study_hours_missing.csv")
...
The gpa_study_hours.csv file is a dataset with 193 observations and 2
variables: gpa and study hours. 193 undergraduate students of an
introductory statistics course were anonymously surveyed in 2012 at a
private US university. GPA ranges from 0 to 4 points; however, one
student reported a GPA greater than 4. This is a data error and was left
in the dataset to illustrate issues with real survey data. Both
variables are self-reported, therefore, may not be accurate.

##### Definition of Data in Dataset

| Variable    | Type | Description                             |
|-------------|------|-----------------------------------------|
| gpa         | num  | Grade point average of student          |
| study_hours | num  | Number of hours students study per week |

##### Summary of Dataset:

```{r, warning=FALSE, echo=T, message=FALSE}
whole_gpa <- gpa_missing %>% 
  select(gpa, study_hours)
tbl_summary(whole_gpa, missing_text = "NA") %>%
  modify_header(label ~ "**Variable**") %>%
  modify_caption("**Summary of GPA Data**") %>%
  bold_labels() %>%
  as_gt() %>% 
...
ggplot(gpa, aes(x=study_hours, y=gpa)) +
...
R, a statistical programming software, we will create multiple datasets
with imputed values for the missing data. We will be deleting some data
points from our dataset to create missing values. We will run through
the MICE process and provide results of the imputations. To check for
accuracy of the imputations, we will then compare these results to the original, complete data set.

##### Evaluate Dataset

First, we evaluate the dataset for missing values. As indicated in the
summary, the data does contain 6 NA/missing values. This equates to about 3% of the data (6/193). If we were to just delete these data points, we would be discarding valuable information when we run regression.

Because our dataset contains 3% of missing data, we will
generate 5 imputations, or 5 new datasets. The MICE package seamlessly
...
First step is to check the missingness by looking for patterns in the
original dataset using the md.pattern() function:

```{r, warning=FALSE, echo=T, message=FALSE}
md.pattern(gpa_missing, rotate.names = TRUE)
```

Blue is observed values and red is missing values.

Using the mice() function, 5 multiple imputations for the missing
values will be generated. This is the default; if you want something different, you must set m = to the
number of imputations that you desire. The
...
```{r, warning=FALSE, echo=T, message=FALSE}
imp = mice(data = gpa_missing, set.seed = 1337)
```

The following R code will show the imputed values. Columns are
imputations, rows are observations.
...
the variable per imputed data set. We want the imputations to be values
that could have been observed had the data not been missing.

```{r, warning=FALSE, echo=T, message=FALSE}
par(mfrow=c(7,2))
stripplot(imp, study_hours, pch = 19, cex=1, xlab = "Imputation number")
```

Next, we will pool the results of the complete dataset with the imputed
dataset to arrive at estimates that will properly account for the
missing data. We fit the complete model with the with() function and
display the summary of the pooled results. It will give us the estimate,
standard error, test statistic, degrees of freedom, and the p-value for
each variable.

```{r, warning=FALSE, echo=T, message=FALSE}
# fit complete-data model
fit <- with(imp, lm(gpa~study_hours))
...
```{r, warning=FALSE, echo=T, message=FALSE}
gpa_lm = lm(gpa ~ study_hours, data = gpa)
summary(gpa_lm)
```
